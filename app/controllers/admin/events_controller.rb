    ...
    if sort.to_i == 1
      if @event_sa.present? && @event_sa.formula.present?
        ...
        if order_num > 1
          ...
          # 单一排序
          sql = "scores.schedule_rank = (select count(*)+1 from (select score from scores s left join teams team on team.id = s.team1_id where team.event_id =#{event_id} and team.group in #{sql_group} and s.schedule_id = #{schedule_id} and s.score #{has_zero} 0) dist_score where dist_score.score #{first_order} scores.score)"
        end
        update_result = Score.joins('inner join teams t on scores.team1_id = t.id').where(event_id: event_id, schedule_id: schedule_id).where("scores.score #{has_zero} ?", 0).where('t.group' => ac_group).update_all(sql)
        if update_result
          flash[:notice] = '排名成功'
        else
          flash[:notice] = '排名失败'
        ...
        if data.length > 0
          if export_type == '1'
            ...
            data = data.map { |score| {
                ...
                成绩属性: score.score_attribute.present? ? show_score_process(JSON.parse(score.score_attribute)) : nil
            } }
          else
          ...
          filename = "#{event_name}_#{params_group}_#{schedule_name}_#{export_type.present? ? '个人' : '队伍'}_#{Time.now.strftime("%Y%m%d%H%M%S")}.xls"
          send_data(data.to_xls, :type => "text/xls;charset=utf-8,header=present", :filename => filename)
        else
          flash[:notice] = '暂无有效成绩'
          redirect_to "/admin/events/scores?id=#{event_id}&group=#{params_group}&schedule=#{schedule_name}"
    ...
    process_hash['rounds_scores'].each_with_index { |r_s, index|
      if r_s == 'invalid'
        ...
        process_hash['process'][index].values.each_with_index { |v|
          if v.is_a?(Hash)
            score_process += v['name'] +':'+v['val'].to_s+';'
          end
        }
