class Admin::NewsController < AdminController
  ...
  def index
    if params[:field].present? && params[:keyword].present?
      ...
      @news = News.left_joins(:admin).where("admins.id=news.admin_id").select(:id, :name, :content, :status, :is_hot, :news_type, :created_at, :updated_at, "admins.name as admin_name").all.page(params[:page]).per(params[:per])
  ...
  def news_params
    params.require(:news).permit(:name, {:news_type => []}, :content, :cover, :desc, :status, :is_hot, :admin_id).tap do |list|
