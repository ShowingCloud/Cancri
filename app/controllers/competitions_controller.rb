class CompetitionsController < ApplicationController
  ...
  def search_team
    ...
    if event_id.present? && team_identify.present?
      team = Team.joins(:event, :school).joins('left join user_profiles up on up.user_id = teams.user_id').where(event_id: event_id, identifier: team_identify).select(:id, :status, :identifier, :players, :teacher, :teacher_mobile, 'events.team_max_num', 'schools.name as school_name', 'up.username')
      result = [true, team]
    else
      result = [false, '参数不完整']
    end
